...
Write-Host "`n📋 Checking current secret exposure..." -ForegroundColor Yellow
...
foreach ($file in $secretFiles) {
    if (Test-Path $file) {
        Write-Host "⚠️  FOUND: $file (contains secrets)" -ForegroundColor Red
    } else {
...
Write-Host "`n💾 Creating backup for key rotation..." -ForegroundColor Yellow
...
Write-Host "`n🗑️  Removing secrets from git tracking..." -ForegroundColor Yellow
...
Write-Host "`n🔄 SECRET ROTATION REQUIRED - Complete these steps:" -ForegroundColor Red
...
Write-Host "`n🚀 CI/CD SECRET INJECTION COMMANDS:" -ForegroundColor Green
...
Write-Host "`n✅ VALIDATION CHECKLIST:" -ForegroundColor Green
...
Write-Host "`n🔒 Security cleanup complete! Check backup folder: $backupDir" -ForegroundColor Green

...
export const getImageWithFallback = (_src: string, fallbackText?: string): string => {
  // If the original src fails to load, this can be used in onError handlers
  const encodedText = encodeURIComponent(fallbackText || 'Product Image');
  // Use placehold.co (more reliable than via.placeholder.com in some networks)
  return `https://placehold.co/600x600?text=${encodedText}&bg=F3F4F6&font=inter&fontweight=700&color=374151`;
};

export const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>, fallbackText?: string) => {
  const target = e.target as HTMLImageElement;
  if (target.src.includes('placeholder')) return; // Avoid infinite loop
  target.src = getImageWithFallback(target.src, fallbackText);
};
